(function() {
    ...
    const processInsertedFeedUnit = (feedUnit, category) => {
        if (category === 'SPONSORED' || setOfCategoriesToHide.has(category)) {
            feedUnit.classList.add(magic); // Hide it right away
        }
    };
    ...
    const checkWhetherFeedUnit = (node) => {
        ...
        const enc = findNestedProperty(feed, /cat.*[sS]ens/, x => findNestedProperty(x, /enc/, x => x, 0), 0);
        const post_id = findNestedProperty(feed, /post_id/, x => x, 2);
        
        if (enc && post_id) {
            fromObfuscatedCategory(node, enc, post_id).then(ctx => {
                if (ctx) processInsertedFeedUnit(ctx.node, ctx.category);
            });
        } else if (!enc) {
            const category = findNestedProperty(feed, /cat.*[sS]ens/, x => findNestedProperty(x, /cat/, x => x, 0), 0);
            if (category) processInsertedFeedUnit(node, category);
        }
    };
    ...
    const start = () => {
        ...
        const observer = new MutationObserver((mutations) => {
            ...
            requestAnimationFrame(() => {
                nodes.forEach(node => checkWhetherFeedUnit(node));
            });
        });
